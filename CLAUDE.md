...
- `src/modules/handlers/http/http.handler.ts` — `email-tracking.httpHandler`: serves `GET /email-tracking/open` (pixel), `GET /email-tracking/attachment-view` (pixel) and `GET /email-tracking/click` (302).
...
- `src/modules/email-tracking/email-tracking.service.ts` — `recordEmailTrackingEvent` (dedupe), `upsertEmailAnalytic` (idempotent projection), `persistTrackedLinks`, `trackEmailOpen`, `trackEmailClick`.
...
- `src/dev-http.ts` — local HTTP server (default :3001) wired to `email-tracking.httpHandler`, for offline tracking E2E.
